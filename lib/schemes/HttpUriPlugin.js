...
class HttpUriPlugin {
	...
	constructor(options) {
		...
		this._keepVersions = options.keepVersions;
	}
			...
			(compilation, { normalModuleFactory }) => {
				...
				const _getCacheKey = url => {
					...
					const moduleName = fromFramer
						? this._keepVersions
							? parsedUrl.pathname // [uniqueId/version/fileName]
							: `${splitted[2]}/${splitted[splitted.length - 1]}` // [uniqueId/fileName]
