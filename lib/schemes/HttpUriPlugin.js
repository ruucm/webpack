...
const computeUniqueId = url => {
	...
	const fromFramer =
		...
		url.includes("framer.com") || url.includes("framerusercontent.com");
	const splitted = parsedUrl.pathname.split("/");

	const moduleName = fromFramer
		? splitted[splitted.length - 1] // if modules from Framer, remove the changing part of the path
		: name.replaceAll("_", "/"); // if modules from self-hosted, keep it's folder structure

	return `${folder.slice(-50)}/${moduleName}`;
};
						...
						fetch(
							...
							res => {
								...
								stream.on("end", () => {
									...
									const integrity = computeUniqueId(url);
...
Run build with un-frozen lockfile to automatically fix lockfile.`
												...
												entry = {
													...entry,
													integrity: computeUniqueId(entry.resolved) // on rebuild (already webpack.lock file exists)
												};
